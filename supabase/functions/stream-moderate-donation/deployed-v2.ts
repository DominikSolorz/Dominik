...
Deno.serve(async(req:Request)=>{
 ...
 const donationId=String(body.donation_id||""); const action=String(body.action||""); const reason=String(body.reason||"").slice(0,500);
 if(!donationId||!["approve","reject"].includes(action))return Response.json({error:"Invalid request"},{status:400,headers:corsHeaders});
 const {data:donation}=await admin.from("stream_donations").select("id,creator_profile_id,voice_url").eq("id",donationId).maybeSingle();
 ...
 const {data:moderator}=await admin.from("stream_moderators").select("can_moderate_messages,can_moderate_voice").eq("profile_id",donation.creator_profile_id).eq("moderator_user_id",userData.user.id).maybeSingle();
 const canModerate=isOwner||Boolean(donation.voice_url?moderator?.can_moderate_voice:moderator?.can_moderate_messages);
